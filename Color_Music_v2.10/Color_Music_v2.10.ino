...
// ----- настройки ленты
#define NUM_LEDS 262        // количество светодиодов (данная версия поддерживает до 410 штук)
...
// ----- отрисовка
#define MODE 5                    // режим при запуске
...
int EMPTY_BRIGHT = 30;           // яркость "не горящих" светодиодов (0 - 255)
#define EMPTY_COLOR HUE_GREEN    // цвет "не горящих" светодиодов. Будет чёрный, если яркость 0
...
#define LOW_COLOR HUE_YELLOW         // цвет низких частот
...
#define STROBE_COLOR HUE_RED   // цвет стробоскопа
#define STROBE_SAT 128              // насыщенность. Если 0 - цвет будет БЕЛЫЙ при любом цвете (0 - 255)
int STROBE_SMOOTH = 200;         // скорость нарастания/угасания вспышки (0 - 255)

// ----- режим подсветки
int LIGHT_COLOR = 0;             // начальный цвет подсветки
int LIGHT_SAT = 255;             // начальная насыщенность подсветки
int COLOR_SPEED = 100;
int RAINBOW_PERIOD = 1;
float RAINBOW_STEP_2 = 0.5;

// ----- режим бегущих частот
int RUNNING_SPEED = 11;
...
void animation() {
  ...
  switch (this_mode) {
    ...
    case 7:
      ...
      break;
    case 8:
      int HUEindex = HUE_START;
      for (int i = 0; i < NUM_LEDS / 2; i++) {
        int this_bright = map(freq_f[(int)floor((NUM_LEDS / 2 - i) / freq_to_stripe)], 0, freq_max_f, 0, 255);
